...
public class SwipeToLoadLayout extends ViewGroup {
    ...
    //低部滚动上来的默认动画时间
    private static final int DEFAULT_DEFAULT_TO_LOADING_MORE_SCROLLING_DURATION = 500;
     ...
     *刷新风格：
     * CLASSIC:有背景，从屏幕外拉进。
     * ABOVE ：无背景，从屏幕外拉进。google风格
     * BLEW：被当前view所覆盖。固定在刷新的位置，下拉时显示出来。
     * SCALE：从中间展开
     */
    private int mStyle = STYLE.ABOVE;

    /**
     * <b>ATTRIBUTE:</b>
     * offset to trigger refresh
     * 下拉到此值刷新，也是当前header停住的位置
     */
    private float mRefreshTriggerOffset=getResources().getDimension(R.dimen.refresh_trigger_offset_google);

    /**
     * <b>ATTRIBUTE:</b>
     * offset to trigger load more
     * 上拉达到此值进行加载,也是当前foot停住的位置
     */
    private float mLoadMoreTriggerOffset=getResources().getDimension(R.dimen.load_more_trigger_offset_google);;
     ...
     * 最大的下拉值
     */
    private float mRefreshFinalDragOffset=getResources().getDimension(R.dimen.refresh_final_offset_google);
     ...
     * 最大的上拉值
     */
    private float mLoadMoreFinalDragOffset=getResources().getDimension(R.dimen.load_more_final_offset_google);
